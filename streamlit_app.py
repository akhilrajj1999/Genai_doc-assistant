...
DATABRICKS_HOST  = "https://dbc-5e299c31-95bc.cloud.databricks.com"
DATABRICKS_TOKEN = os.environ.get("DATABRICKS_TOKEN", "")

if not DATABRICKS_TOKEN:
    st.error("DATABRICKS_TOKEN environment variable is not set. Configure it in Databricks App settings.")
    st.stop()
import streamlit as st
import os, json
...
def load_chains():
    ...
    # Databricks Apps automatically injects these for the app's
    # built-in service principal — no manual token needed
    host  = os.environ.get("DATABRICKS_HOST", "https://dbc-5e299c31-95bc.cloud.databricks.com")
    token = os.environ.get("DATABRICKS_TOKEN")

    # If no token injected, try SDK auto-auth (works when App SP is configured)
    if not token:
        try:
            w     = WorkspaceClient()
            token = w.config.token
            host  = w.config.host
            print("✅ Auth via SDK auto-config")
